...
export const AuthProvider = props => {
	...
	const {
		...
		memoized.signin = async (email, password) => {
			await sign('/v1/auth/signin', email, password);
			history.push('/');
		};

		memoized.signup = async (email, password) => {
			await sign('/v1/auth/signup', email, password);
			history.push('/');
		};
